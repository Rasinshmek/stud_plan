...
public class OtherServices {
    ...
    public List<AnketDTO> getAnketDTOs2() {
        System.out.println("Получение анкет");
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        String token = getTokenFromAuthService2();
        System.out.println("Token:" + token);
        headers.add("Authorization", "Bearer " + token);
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<>(headers);
    ...
    private String getTokenFromAuthService2() {
        ...
        ResponseEntity<String> response = restTemplate.exchange("https://ebook.vstu.by/authorization/token?grant_type=password",
